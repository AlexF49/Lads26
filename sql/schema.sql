...
-- Greensomes only: holes where the single player called a Gruesome (forcing the pair to
-- take their worse drive). 2 per round, tracked the same way as drives — presence of a row
-- means "called this hole".
create table gruesomes (
  id uuid primary key default gen_random_uuid(),
  match_id uuid references matches(id) on delete cascade,
  hole integer not null check (hole between 1 and 18),
  created_at timestamptz default now(),
...
alter table gruesomes enable row level security;
...
create policy "public read" on gruesomes for select using (true);
...
create policy "public write insert" on gruesomes for insert with check (true);
create policy "public write update" on gruesomes for update using (true);
create policy "public write delete" on gruesomes for delete using (true);
...
alter publication supabase_realtime add table gruesomes;
